package fr.gabinbrochard.pendu.application;
...
//	private char[] lettreEsseyer;
	private ArrayList<Character> lettresEssayees = new ArrayList<>();
//	private String lettresEsseyes = "";
	...
	private static final String[] LISTE_MOTS = { "chat", "chien", "maison", "ordinateur", "fleur", "soleil", "banane",
			"montagne", "musique", "plage" };
	...
	public void choisirMot() {
		...
		Random r = new Random();
		int numMot;
		numMot = r.nextInt(LISTE_MOTS.length - 1);

		// Sélection du mot
		char[] mot = new char[0];
		mot = LISTE_MOTS[numMot].toUpperCase().toCharArray();
		setMotATrouver(mot);
	...
	public void tentative(char lettre) {
		...
		if (existe) {
			...
			if (lettreDejaEssayee(lettre)) {
				System.err.println("Tu à déjà saisi cette lettre !");
			} else {
	...
	private boolean lettreDejaEssayee(char lettre) {
	...
	public void afficherInfoPartie() {
		if (!lettresEssayees.isEmpty()) {
			String listeLettres = "";
			for (char lettre : lettresEssayees) {
				listeLettres = (listeLettres == "") ? String.valueOf(lettre) : listeLettres + " - " + lettre;
			}
			System.out.println("Lettre esseyée.s : " + listeLettres + "\n\n");
		}
	...
	public void finParti() {
		if (getNbTentativeErr() < 9) {
			// GAGNÉ
			System.out.println("Félicitation tu à trouvé le mot en " + getNbTentative() + " tentative.s !");
			setFini(true);
		}
	}
